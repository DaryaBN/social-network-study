...
export const postHashtag = createAsyncThunk(
	"counter/hashtag",
	async ({ hashtagResult }, { rejectWithValue }) => {
		try {
			const response = await fetch("/hashtagPosts", {
				method: "POST",
				headers: {
					"Content-Type": "application/json;charset=utf-8",
				},
				body: JSON.stringify({ hashtagt: hashtagResult }),
			});
			if (response.ok) {
				const data = await response.json();
				return data;
...
const counterSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(newsFeed.pending, (state) => {
			state.status = "loading";
			state.error = null;
		});
		builder.addCase(newsFeed.fulfilled, (state, action) => {
			state.status = "resolved";
			state.posts = action.payload;
		});
