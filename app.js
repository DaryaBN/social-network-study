...
const pool = new Pool({
	...
	port: 6543,
  idleTimeoutMillis: 30000, // тайм-аут бездействия
  connectionTimeoutMillis: 2000, // тайм-аут при подключении
	ssl: {
...
pool.on('error', (err) => {
  console.error('КРИТИЧЕСКАЯ ОШИБКА ПУЛА:', err.message);
});
...
let information = await pool.query(`
  SELECT p.id, p.id_user, p.username, p.usernick, p.time, p.mess, p.img, u.photo
  FROM posts p
  JOIN usersinfo u ON p.id_user = u.user_id
...
app.post('/login', async (req, res) => {
	...
	const sessionResult = await pool.query(
		"SELECT * FROM sessions WHERE email = $1 ORDER BY date DESC LIMIT 1", 
		[email]
	);
		const lastSession = sessionResult.rows[0];
		const now = new Date();
		let token = crypto.randomUUID();
		if (lastSession) {
			const days = (now.getTime() - new Date(lastSession.date).getTime()) / 86400000;
			if (days <= 1) {
				token = lastSession.token; 
			} else {
				await pool.query(
					"INSERT INTO sessions (id_user, date, email, token) VALUES ($1, $2, $3, $4)",
					[user.id, now, email, token]
				);
			}
		} else {
			await pool.query(
		...
		res.cookie('token', token, { maxAge: 86400000, secure: true });
		res.cookie('email', email, { maxAge: 86400000, secure: true });

		res.status(200).json([user]);

	} catch (err) {
		console.error("Ошибка в /login:", err.message);
...
app.get('/DataMessToday', async (req, res) => {
	try {
		const result = await pool.query("SELECT COUNT(*) FROM posts WHERE time::text LIKE $1", [`%${new Date().toDateString()}%`]);
		res.type('text').send(String(result.rows[0].count));
	} catch (err) {
		res.send("0");
	}
});
...
app.post('/subscriptionNow', async (req, res) => {
	try {
		...
		if (!emailCook) return res.send('читать');
		...
		const result = await pool.query(`
			...
			JOIN users u ON s.follower_id::text = u.id::text
			WHERE u.email = $1 AND s.user_id::text = $2
		`, [email, userId]);

		if (result.rows.length > 0) {
			res.status(200).send('читаю');
		} else {
			res.status(200).send('читать');
		}
	} catch (err) {
		console.error("Ошибка в /subscriptionNow:", err.message);
		res.send('читать');
	}
});
...
app.get("/recommendations", async (req, res) => {
	try {
		const cookEmailRaw = req.cookies.email;
		const cookEmail = cookEmailRaw ? cookEmailRaw.replace(/'/g, "").trim() : null;
		let query;
		let params = [];
		if (cookEmail) {
			query = `
				SELECT ui.id, ui.user_id, ui.username, ui.usernick, ui.photo 
				FROM usersinfo ui
				JOIN (
					SELECT user_id::text, COUNT(*) as count 
					FROM subscriptions 
					WHERE user_id::text != (SELECT id::text FROM users WHERE email = $1)
					GROUP BY user_id
					ORDER BY count DESC 
					LIMIT 3
				) s ON ui.user_id::text = s.user_id
			`;
			params = [cookEmail];
		} else {
			query = `
		...
		res.type("json").send(result.rows);
	} catch (err) {
		console.error("Ошибка в /recommendations:", err.message);
		res.json([]); // Возвращаем пустой массив при ошибке, чтобы сайт не падал
	}
});
...
app.post('/LikePost', async (req, res) => {
	try {
		...
		if (!idPost) return res.send("0");

		const result = await pool.query(
			'SELECT COUNT(*) AS count FROM likes WHERE post_id::text = $1::text', 
			[String(idPost)]
		);
		const count = result.rows[0].count;
		res.status(200).send(String(count));
	} catch (err) {
		console.error("Ошибка в /LikePost:", err.message);
		if (!res.headersSent) {
