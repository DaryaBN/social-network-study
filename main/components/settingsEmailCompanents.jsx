...
const EmailComponent = () => {
	...
	const handleSubmit = (e) => {
		...
		if (
			usersEmail.email.length !== 0 &&
			validationEmail(usersEmail.email) === true
		) {
			e.preventDefault();
			const newEmail = {
	...
	return (
		<form onSubmit={handleSubmit}>
			<div className="SettingsInform">
				<div
					className={errorColorEmail ? "EditInput" : "EditInput EditInputError"}
				>
					...
					<div className="EditValue">
						<input
							type="text"
							name="email"
							value={usersEmail.email}
							onChange={handleChange}
						/>
					</div>
					<div className="TextNickError">
						{textErrorValdation ? "" : "Адрес не валиден"}
					</div>
					<div className="TextNickError">
						{textErrorLenght ? "" : "Поле не заполненно"}
					</div>
				</div>
				<div
					className={errorColorPass ? "EditInput" : "EditInput EditInputError"}
				>
					...
					<div className="EditValue">
						<input
							type="text"
							name="password"
							value={usersEmail.password}
							placeholder="*******************"
							onChange={handleChange}
						/>
					</div>
					<div className="TextNickError">
						{textErrorPass ? "" : "Неверный пароль"}
					</div>
				</div>
			</div>
			...
			{visible && (
				<div
					className={`SettingsResult show ${
						textNewEmail !== "Email успешно изменен"
							? "SettingsResultRed"
							: textNewEmail === "Email успешно изменен"
								? "SettingsResultGreen"
								: ""
